...
impl CheckpointManager {
    ...
    pub fn find_best_checkpoint(&self, active_colliders: &HashSet<u64>) -> Option<&DynamicTileCheckpoint> {
        self.checkpoints
            .iter()
    ...
    pub fn cleanup_redundant_checkpoints(&mut self) {
        ...
        for (i, checkpoint) in self.checkpoints.iter().enumerate() {
            ...
            let is_redundant = self.checkpoints[0..i].iter().any(|newer| {
                checkpoint.colliders.is_subset(&newer.colliders)
            });
