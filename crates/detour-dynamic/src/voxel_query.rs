...
impl VoxelQuery {
    ...
    fn traverse_tiles(&self, start: Vec3, end: Vec3) -> Option<VoxelRaycastHit> {
        ...
        let x_offset = if tx < 0.0 { x_rem } else { self.tile_width - x_rem }.abs();
        let z_offset = if tz < 0.0 { z_rem } else { self.tile_depth - z_rem }.abs();
        
        let tx_abs = tx.abs();
        let tz_abs = tz.abs();
        
        let mut t_max_x = if tx_abs > 0.0 { x_offset / tx_abs } else { f32::INFINITY };
        let mut t_max_z = if tz_abs > 0.0 { z_offset / tz_abs } else { f32::INFINITY };
        
        let t_delta_x = if tx_abs > 0.0 { self.tile_width / tx_abs } else { f32::INFINITY };
        let t_delta_z = if tz_abs > 0.0 { self.tile_depth / tz_abs } else { f32::INFINITY };
        ...
        loop {
            ...
            if let Some(hit) = self.traverse_heightfield(current_x, current_z, start, end, t, next_t) {
                return Some(hit);
            }
            
            // Check if we've reached the end tile
            if (if dx > 0 { current_x >= ex } else { current_x <= ex }) &&
               (if dz > 0 { current_z >= ez } else { current_z <= ez }) {
                break;
            }
    ...
    fn traverse_heightfield(
        ...
        let x_offset = if tx < 0.0 { x_rem } else { heightfield.cs - x_rem }.abs();
        let z_offset = if tz < 0.0 { z_rem } else { heightfield.cs - z_rem }.abs();
        ...
        let t_delta_x = if tx_abs > 0.0 { heightfield.cs / tx_abs } else { f32::INFINITY };
        let t_delta_z = if tz_abs > 0.0 { heightfield.cs / tz_abs } else { f32::INFINITY };
        ...
        loop {
            ...
            if current_x >= 0 && current_x < heightfield.width && 
               current_z >= 0 && current_z < heightfield.height {
...
mod tests {
    ...
    fn test_voxel_query_creation() {
        let query = VoxelQuery::from_single_heightfield(
            Vec3::new(0.0, 0.0, 0.0),
            4.0, 4.0,
        );
