...
use waymark::{NavMesh, NavMeshParams};
...
impl DynamicNavMesh {
    ...
    pub fn update_nav_mesh(&mut self) -> Result<bool, DynamicError> {
        ...
        let has_built_tiles = self
            .tiles
            .values()
            .any(|tile| tile.is_built() && tile.has_mesh_data());
    ...
    pub fn get_statistics(&self) -> DynamicNavMeshStatistics {
        let dirty_tiles = self.tiles.values().filter(|t| t.needs_rebuild()).count();

        let memory_usage = self.tiles.values().map(|t| t.memory_usage()).sum();
    ...
    fn get_tiles_by_bounds(&self, bounds: &[f32; 6]) -> Vec<&DynamicTile> {
        let minx = ((bounds[0] - self.nav_mesh_params.origin[0]) / self.nav_mesh_params.tile_width)
            .floor() as i32
            - 1;
        let minz = ((bounds[2] - self.nav_mesh_params.origin[2]) / self.nav_mesh_params.tile_height)
            .floor() as i32
            - 1;
        let maxx = ((bounds[3] - self.nav_mesh_params.origin[0]) / self.nav_mesh_params.tile_width)
            .floor() as i32
            + 1;
        let maxz = ((bounds[5] - self.nav_mesh_params.origin[2]) / self.nav_mesh_params.tile_height)
            .floor() as i32
            + 1;
    ...
    fn intersects_xz(&self, tile: &DynamicTile, bounds: &[f32; 6]) -> bool {
        tile.bounds_min.x <= bounds[3]
            && tile.bounds_max.x >= bounds[0]
            && tile.bounds_min.z <= bounds[5]
            && tile.bounds_max.z >= bounds[2]
    }
    ...
    pub async fn build_async(&mut self) -> Result<bool, DynamicError> {
        ...
        let tiles_to_build: Vec<i64> = self
            .tiles
            .iter()
            .filter_map(|(key, tile)| {
                if tile.needs_rebuild() {
