...
use glam::Vec3;
use landmark::{CompactHeightfield, ContourSet, Heightfield, PolyMesh, PolyMeshDetail};
...
impl DynamicTile {
    ...
    fn build_compact_heightfield(&mut self) -> Result<(), DynamicError> {
        if let Some(ref heightfield) = self.heightfield {
            ...
            let compact_hf = landmark::CompactHeightfield::build_from_heightfield(
                heightfield,
                walkable_height,
                walkable_climb,
            )?;
        ...
        if let Some(ref mut compact_hf) = self.compact_heightfield {
            match self.config.partition {
                ...
                1 => {
                    ...
                    landmark::build_regions_monotone(
                        compact_hf,
                        0, // border_size
                        self.config.min_region_area as i32,
                        self.config.region_merge_area as i32,
    ...
    fn build_contours(&mut self) -> Result<(), DynamicError> {
        if let Some(ref compact_hf) = self.compact_heightfield {
            let contour_set = landmark::ContourSet::build_from_compact_heightfield(
                compact_hf,
    ...
    fn build_poly_mesh(&mut self) -> Result<(), DynamicError> {
        if let Some(ref contour_set) = self.contour_set {
            let poly_mesh = landmark::PolyMesh::build_from_contour_set(
                contour_set,
    ...
    fn build_detail_mesh(&mut self) -> Result<(), DynamicError> {
        if let (Some(compact_hf), Some(poly_mesh)) = (&self.compact_heightfield, &self.poly_mesh) {
            let detail_mesh = landmark::PolyMeshDetail::build_from_poly_mesh(
                poly_mesh,
                compact_hf,
    ...
    pub fn create_nav_mesh_data(
        &self,
        nav_mesh_params: &waymark::NavMeshParams,
        ...
        use waymark::{NavMeshBuilder, NavMeshCreateParams, PolyFlags};
