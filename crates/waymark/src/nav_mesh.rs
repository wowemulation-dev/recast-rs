...
use landmark::{MESH_NULL_IDX, PolyMesh, PolyMeshDetail};
...
pub struct MeshTile {
    ...
    pub(crate) flags: u8,
    /// Next free tile in the free-list
...
impl NavMesh {
    ...
    ///
    /// ```no_run
    /// use waymark::{NavMesh, NavMeshParams, QueryFilter};
    ///
    /// # fn example(nav_mesh: &NavMesh) -> Result<(), Box<dyn std::error::Error>> {
    /// let filter = QueryFilter::default();
    /// let bmin = [10.0, 0.0, 10.0];
    /// let bmax = [20.0, 5.0, 20.0];
    /// let polys = nav_mesh.query_polygons_in_tile(5, 3, &bmin, &bmax, &filter)?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// # Examples
    /// ```no_run
    /// use waymark::{NavMesh, NavMeshParams, PolyFlags, PolyRef};
    ///
