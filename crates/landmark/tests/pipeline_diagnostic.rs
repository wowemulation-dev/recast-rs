...
//! Run with: cargo test -p landmark --test pipeline_diagnostic -- --nocapture
...
fn run_pipeline_diagnostic(obj_name: &str) {
    ...
    for z in 0..heightfield.height() {
        for x in 0..heightfield.width() {
            ...
            if si != landmark::SPAN_NULL {
                cells_with_spans += 1;
            }
            while si != landmark::SPAN_NULL {
...
fn count_walkable_spans(hf: &landmark::Heightfield) -> (u64, u64) {
    let mut total = 0u64;
...
///
/// Run: cargo test -p landmark --test pipeline_diagnostic filter_stages -- --nocapture
#[test]
fn filter_stages_nav_test() {
    ...
    let mut hf = landmark::Heightfield::new(
        config.width,
        config.height,
        bmin,
        bmax,
    ...
    for i in 0..num_triangles {
        ...
        landmark::rasterize_triangle(&v0, &v1, &v2, area, &mut hf, 1).expect("rasterize failed");
    }
